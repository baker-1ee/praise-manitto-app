...
export default function TeamManageScreen() {
  ...
  const handleRegenerate = () => {
    Alert.alert(
      '초대 코드 재발급',
      '기존 코드는 더 이상 사용할 수 없습니다. 재발급하시겠어요?',
      [
      ...
      { text: '취소', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert('오류', e instanceof Error ? e.message : '팀 나가기에 실패했습니다.');
          } finally {
            setLeaving(false);
          }
        },
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={loadingData} onRefresh={loadData} tintColor={AppColors.primary} />}
      >
        {/* 초대 코드 섹션 (LEADER만) */}
        {isLeader && team && (
            ...
            <View style={styles.codeCard}>
              ...
              <View style={styles.codeActions}>
                <TouchableOpacity style={styles.codeBtn} onPress={handleCopyCode}>
                  <IconSymbol name="checkmark" size={14} color={copied ? AppColors.success : AppColors.primary} />
                ...
                <TouchableOpacity style={styles.codeBtn} onPress={handleRegenerate} disabled={regenerating}>
                  {regenerating ? (
                    ...
                    <IconSymbol name="arrow.right.square" size={14} color={AppColors.textMuted} />
                  )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.membersList}>
            {members.map((item) => (
              <React.Fragment key={item.membershipId}>
                {renderMember({ item })}
              </React.Fragment>
            ))}
          </View>
