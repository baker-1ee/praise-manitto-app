...
### BUG-001 ✅ 해결됨
**제목**: 홈 탭 "내가 받은 칭찬" 누르면 칭찬 탭의 "보낸 칭찬" 탭으로 이동  
**증상**: 홈 탭에서 "내가 받은 칭찬" 통계 카드를 누르면 칭찬 탭(`/praises`)으로 이동은 하지만, 기본 탭인 "보낸 칭찬"이 선택된 채로 열림  
**기대 동작**: 받은 칭찬 통계 카드 누를 시 → 칭찬 탭의 "받은 칭찬" 탭이 선택된 상태로 이동  
**원인**: `index.tsx`에서 두 카드 모두 `router.push('/(tabs)/praises')`로 탭 파라미터 없이 이동. `praises.tsx`는 항상 `'sent'`를 기본 탭으로 초기화  
**수정**: `index.tsx` "내가 받은 칭찬" 카드에 `params: { tab: 'received' }` 추가. `praises.tsx`에서 `useLocalSearchParams`로 `tab` 파라미터 읽어 초기 탭 설정
