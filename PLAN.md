...
- [ ] 스프린트 생성 화면 (LEADER, 이름/시작일/종료일)
- [ ] Fisher-Yates derangement 클라이언트 실행 → `sprints/{id}/pairs` 저장
- [ ] 스프린트 목록 (LEADER 팀 관리 화면 내)
- [ ] 홈 화면 (`/`)
  - [ ] 팀 선택 헤더 (드롭다운)
  - [ ] 현재 스프린트 현황 카드
  - [ ] 내 마니또 카드 (플립 애니메이션 — 앞면: "마니또 배정됨", 뒷면: 대상 이름/소개)
  - [ ] 보낸/받은 칭찬 수 통계 카드
  - [ ] 진행 중 스프린트 없을 때 직전 공개 스프린트 카드

**산출물**: `app/(app)/index.tsx`, `app/(app)/team/[teamId]/manage.tsx` (스프린트 섹션), `lib/manito.ts`, `components/manito-card.tsx`
...
- [ ] 칭찬 작성 화면 (`/praise/write`)
  - [ ] 내 마니또 대상 자동 지정 (현재 팀 스프린트 기준)
  - [ ] 10~500자 입력, 글자 수 카운터
  - [ ] 카테고리 최대 3개 선택
  - [ ] 전송 완료 후 보낸 칭찬 목록으로 이동
- [ ] 칭찬 목록 화면 (`/praises`)
  - [ ] 보낸 칭찬 탭 (현재 팀 스프린트 기준 필터)
  - [ ] 받은 칭찬 탭 (공개 전 발신자 "익명의 팀원" 표시)
- [ ] 칭찬 조르기 버튼 (홈 화면)
  - [ ] `nudgeLogs`에 당일 기록 여부 체크 (하루 1회 제한)
  - [ ] 조르기 성공 시 `nudgeLogs` 저장

**산출물**: `app/(app)/praise/write.tsx`, `app/(app)/praises/index.tsx`, `components/praise-nudge-button.tsx`, `lib/praises.ts`
...
- [ ] LEADER 팀 관리 화면 — 공개 버튼
  - [ ] 공개 전 미칭찬 멤버 검사:
    - `pairs` 전체 순회 → 각 `manitoId`별 praise 존재 여부 확인
    - 미작성자 있으면 경고 모달 + 이름 목록 표시, 공개 차단
    - 모두 작성했을 때만 `status = 'REVEALED'` 업데이트
  - [ ] 미칭찬 멤버 현황 목록 (스프린트 진행 중 항시 표시)
- [ ] 공개 결과 화면 (`/reveal/[sprintId]`)
  - [ ] 진입 시 confetti 효과
  - [ ] 스프린트 이름 + 기간 + 총 칭찬 수
  - [ ] 멤버별 카드: 아바타 + "○○님의 마니또는 △△님" + 칭찬 목록
  - [ ] 카드 클릭 시 펼침/접힘 토글
  - [ ] 로그인 없이도 접근 가능 (팀원이 링크 공유 시)

**산출물**: `app/(app)/reveal/[sprintId].tsx`, `lib/celebration.ts`, `components/reveal-card.tsx`
...
- [ ] 로딩 스켈레톤 (칭찬 목록, 마니또 카드)
- [ ] 빈 상태 화면 (스프린트 없음, 칭찬 없음)
- [ ] 에러 화면 (네트워크 오류, 권한 없음)
- [ ] Firestore 실시간 리스너 (칭찬 수신 시 목록 자동 갱신)
- [ ] 앱 아이콘 / 스플래시 스크린 적용
- [ ] 다크/라이트 테마 완성도 점검
